...
class VideoPoker:
    ...
    def display_hand(self, hold_indices=[]):
        print("\n▓▒░ YOUR HAND ░▒▓")
        print(" ".join(f"[{card}]" for card in self.hand))  # Cards with no extra spaces
        print(" ".join("[X]" if i in hold_indices else "[ ]" for i in range(5)))  # Align [X] properly under cards
    ...
    def play_round(self, input_func):
        ...
        if hold_input.strip():
            try:
                hold_indices = [int(i) - 1 for i in hold_input.split() if i.strip().isdigit()]  # Space-separated input for hold
            except ValueError:
